...
- **在 Obsidian 中搜索全部收藏：** 无需逐个打开文件，即可搜索标题、正文、代码、
  已加载评论和收藏备注。
- **让 AI 和文本工具读懂收藏库：** 可选择生成一个 Markdown 资料库，供 Codex、
  ripgrep、Dataview 和普通 AI 插件读取，不会为每篇收藏创建一份旁挂笔记。
- **继续保留网页体验：** 离线页面仍可保留支持的图片、轮播、评论、排版、链接和
  一键复制代码功能。
- **兼容旧收藏：** 已有 `.pagenest` 和旧 `.hermes` 文件仍可打开。详情见
  [v1.9.0 更新说明](docs/release-notes-v1.9.0.md)。
...
**保存下来的不只是“能读的文字”，而是仍然好用的离线网页：**

- **网页模式阅读：** 尽力保留正文结构、图片、已加载评论、表格、代码和网页排版，
  而不是把所有内容压成纯文本。
- **Viewer 交互：** 在 Obsidian 内直接打开，浏览支持的图片轮播、访问链接，并一键复制
  普通正文或代码块。
- **一个可移动文件：** 页面与可嵌入资源一起保存在单个 `.pagenest` 文件中，不产生
  旁挂的附件目录。
- **可选 AI 整理：** 完整离线网页始终保留；需要时再使用自己配置的 OpenAI 兼容接口
  生成文字总结或图文总结，AI 失败也不会丢失原始收藏。

## PageNest 会保留什么

- 文章结构、原位图片、GIF、表格、代码块、链接、支持的媒体和收藏备注。
- 为 GitHub README、小红书、古月居、飞书、微信、CSDN、B站提供专用适配，其他网站使用通用文章采集。
...
| 模式 | 保存结果 | 是否需要 AI |
| --- | --- | --- |
| 完整保存网页 | 原文、图片、代码、链接和尽力保留的网页排版 | 否，推荐默认使用 |
| 完整保存 + AI 文字总结 | 完整网页，并附加基于正文的文字总结 | 是 |
| 完整保存 + AI 图文总结 | 完整网页，并让支持视觉的模型结合正文与图片总结 | 是 |
...
| 网站或页面类型 | 当前支持 |
| --- | --- |
| GitHub README | README 正文与图片；清理标题锚点链接，只保留一个来源页跳转入口 |
