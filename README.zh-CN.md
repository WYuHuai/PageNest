...
- 支持收藏用户在 Edge/Chrome 当前标签页中主动打开的本地 HTML；需要用户开启
  “允许访问文件网址”，本地绝对路径不会发送给 Service。
...
| 浏览器中打开的本地 HTML | 当前渲染后的正文、结构、代码、表格、链接和浏览器可读取图片 |
...
| `<all_urls>` | 支持任意文章网站、页面内签名资源，以及用户另行授权后的当前本地 HTML 标签页 |
