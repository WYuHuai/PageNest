...
(async () => {
  ...
  const root = {
    querySelectorAll(selector) {
      ...
      if (selector === "a[href]") return [localLink, anchorLink, remoteLink];
  ...
  const context = vm.createContext({
    ...
    fetch: async url => {
      requested.push(url);
      if (url.endsWith("missing.png")) throw new Error("unreadable local image");
      return {
        ok: true,
        blob: async () => ({
          size: 128,
  ...
  const stats = await context.HermesExtractorCore.inlineLocalImages(images);
  assert.deepEqual(Array.from(requested), [
    "file:///D:/AI/images/figure.png",
    "file:///D:/AI/images/missing.png",
  ]);
  assert.deepEqual(JSON.parse(JSON.stringify(stats)), {attempted: 2, inlined: 1, failed: 1});
  ...
  assert.equal(failedNode.getAttribute("src"), "");
  ...
  assert.match(images[1].resolved_url, /^local-resource-unavailable:\/\/image\//);
